...
class MyGraph:
    ...
    def singleSourceShortestPath(self, srcID):
        ...
        if srcID == None or self.adjList[srcID] == None:    #make sure srcId is not None
            return
        Dist = 0.0
        ...
        while(not pq.isEmpty()):    #check if the queue is empty
            j = pq.extractMin()     #tuple of min distance and vertex. j = (vertex,minDist)
            for i in self.adjList[j[0]]:
                if pq.hasKey(i[0]):     #assertion of pq.get(i) is that i must be in the priority queue
                    if i[1] + j[1] < pq.get(i[0]):  #if distance of prior two nodes is less than current distance
                        Dist = j[1] + i[1]
